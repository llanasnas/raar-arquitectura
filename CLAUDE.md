...
| `app/page.tsx` | **La portada** (= la propuesta C desde el 17/09/2026): apertura de persiana + `Cover` + `Diagram` + `Feature` + las cuatro propuestas del estudio (A–D, hasta que el cliente elija) + `Closing` + `SiteFoot`. `/home-c` redirige aquí |
...
| `components/site/v2/Diagram.tsx` + `lib/diagram.ts` | **El diagrama de obra** de la home C: SVG en línea, cinco arcos (tipologías) en tres anillos, códigos de obra en el perímetro con su línea, isotipo en el centro (→ estudio). La lista de qué obra va en qué tipología vive en `lib/diagram.ts`; lo que no tiene ficha se dibuja sin enlace |
...
| `components/site/v2/About.tsx` + `AboutD.tsx` | **El estudio en la home, cuatro propuestas a elegir** (17/09): A pliego (foto pegada + manifiesto con el primer párrafo grande), B tres columnas con rótulo (`studioPage.themes`: Origen · Materia · Proceso) y lámina, C cita en grande (`studioPage.quote`) + cinta de las cuatro palabras + texto a dos columnas, D al hilo del texto (lámina pegada que cambia con el párrafo que cruza el centro, IntersectionObserver). `AboutMark` es el marcador A/B/C/D, temporal. La foto del equipo es de 640 px: nunca a sangre |
...
- **Menú**: **en fila** a lo largo del borde inferior empezando por la izquierda, sin fondo, en caja alta. Inicio · Proyectos · Estudio · Contacto. En móvil los cuatro tienen que caber en **una** línea (0,85rem): si se parten, la segunda sube hasta el logotipo. El logotipo también enlaza a la portada. Referencia: `estudiodiir.com` (capturas en `shots/ref-diir-*.png`); segunda referencia `mesura.eu`.
...
- Menú abajo a la izquierda, sin fondo, con Inicio · Proyectos · Estudio · Contacto.
...
Datos legales y de socios · fotos de obra terminada y testimonios · nombres públicos, año, m² y estado de cada proyecto · material de BO24, CM25 y LL14 (sin ficha: en el diagrama van sin enlace) · sección «Off grid» (eventos y camisetas, tipo boletín; no existe aún) · **servicios a cinco** (Obra nueva, Rehabilitación, Project management, Dirección de obra, Interiorismo: faltan textos de los tres nuevos) · si IM10 y TO39 van en la home · **elegir propuesta del estudio (A–D) en la home** · foto del equipo a más resolución (la que hay es de 640 px) · GV75/SE08/PR37/GR16 · CA/EN (estructura preparada en `lib/copy.ts` y `content/projects/es/`) · Resend en producción · 301 desde `/works.html`, `/worksXX.html`.
